...
def triton_per_fused_index_mul_sum_0(in_ptr0, out_ptr0, xnumel, rnumel, XBLOCK : tl.constexpr):
    ...
    tmp183 = tl.load(in_ptr0 + (tmp182 + (200000*x2)), rmask, eviction_policy='evict_last', other=0)
...
def check_x_indices_values():
    import math

    from torch._decomp.decompositions import _compute_indices_weights_aa
    ...
    kx = torch.arange(x_max_interp_size, device="cpu")
    src_x_min = src_x_min.unsqueeze(dim=-1)
    x_indices = torch.clamp(src_x_min + kx, max=400 - 1)
...
if __name__ == "__main__":
    ...
    with torch.cuda._DeviceGuard(0):
        ...
        output = empty((1, 3, 256, 256), device='cuda', dtype=torch.float32)
