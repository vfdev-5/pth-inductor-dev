...
# kernel path: /tmp/torchinductor_root/mv/cmvu6bxfdth4fqqcnxv35xy77hyfcjrq2r4zoo7dfgvrdywsvjlp.py
...
@pointwise(size_hints=[524288], filename=__file__, meta={'signature': {0: '*fp32', 1: '*fp32', 2: '*fp32', 3: 'i32'}, 'device': 0, 'constants': {}, 'mutated_arg_names': ['in_out_ptr0'], 'autotune_hints': set(), 'configs': [instance_descriptor(divisible_by_16=(0, 1, 2), equal_to_1=())]})
@triton.jit
def triton_(in_out_ptr0, in_ptr0, in_ptr1, xnumel, XBLOCK : tl.constexpr):
    ...
    tmp29 = tl.load(in_ptr1 + (x1 + (3*tmp27) + (1368*tmp24)), xmask)
    tmp30 = 1.0
    tmp31 = tmp5 + tmp30
    tmp32 = tmp31 - tmp4
    tmp33 = tmp15 + tmp30
    ...
    tmp46 = tl.load(in_ptr1 + (x1 + (3*tmp44) + (1368*tmp41)), xmask)
    tmp47 = tmp4 - tmp5
    tmp48 = tmp47 * tmp34
    ...
    tmp60 = tl.load(in_ptr1 + (x1 + (3*tmp58) + (1368*tmp56)), xmask)
    tmp61 = tmp14 - tmp15
    tmp62 = tmp32 * tmp61
    ...
    tmp77 = tl.load(in_ptr1 + (x1 + (3*tmp69) + (1368*tmp68)), xmask)
    tmp78 = tmp77 * tmp67
    tmp79 = tmp74 + tmp78
