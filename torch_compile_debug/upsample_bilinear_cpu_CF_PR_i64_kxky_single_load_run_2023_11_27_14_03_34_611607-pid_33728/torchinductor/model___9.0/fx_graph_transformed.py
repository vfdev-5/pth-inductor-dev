class <lambda>(torch.nn.Module):
    def forward(self, arg0_1: "u8[2, 3, 500, 500]"):
        ...
        view_1: "f32[256, 2, 1, 1]" = torch.ops.aten.reshape.default(cat_1, [256, 2, 1, 1]);  cat_1 = None
        ...
        view_2: "f32[1, 256, 2]" = torch.ops.aten.reshape.default(cat, [1, 256, 2]);  cat = None
        unsqueeze_1: "i32[256, 1]" = torch.ops.aten.unsqueeze.default(convert_element_type_1, -1);  convert_element_type_1 = None
        iota_3: "i64[2]" = torch.ops.prims.iota.default(2, start = 0, step = 1, dtype = torch.int64, device = device(type='cpu'), requires_grad = False)
        add_3: "i64[256, 2]" = torch.ops.aten.add.Tensor(unsqueeze_1, iota_3);  unsqueeze_1 = iota_3 = None
        clamp_max_1: "i64[256, 2]" = torch.ops.aten.clamp_max.default(add_3, 499);  add_3 = None
        view: "i64[256, 2, 1, 1]" = torch.ops.aten.reshape.default(clamp_max_1, [256, 2, 1, 1]);  clamp_max_1 = None
        unsqueeze: "i32[256, 1]" = torch.ops.aten.unsqueeze.default(convert_element_type, -1);  convert_element_type = None
        iota_2: "i64[2]" = torch.ops.prims.iota.default(2, start = 0, step = 1, dtype = torch.int64, device = device(type='cpu'), requires_grad = False)
        add_2: "i64[256, 2]" = torch.ops.aten.add.Tensor(unsqueeze, iota_2);  unsqueeze = iota_2 = None
        clamp_max: "i64[256, 2]" = torch.ops.aten.clamp_max.default(add_2, 499);  add_2 = None
