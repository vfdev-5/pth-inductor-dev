...
def call(args):
    ...
    with torch.cuda._DeviceGuard(0):
        ...
        triton_per_fused_index_mul_sum_0.run(arg0_1, buf6, 256 * 256 * 3 * 4, 25, grid=grid(256 * 256 * 3 * 4), stream=stream0)
...
def call_v2(args):
    ...
    with torch.cuda._DeviceGuard(0):
        ...
        triton_per_fused_index_mul_sum_0_v2.run(arg0_1, buf6, 256 * 256, 25, grid=grid(256 * 256), stream=stream0)

        del arg0_1
        return (buf6, )


def benchmark_compiled_module(times=15000, repeat=500):
...
def benchmark_compiled_module_v2(times=15000, repeat=500):
    from torch._dynamo.testing import rand_strided
    from torch._inductor.utils import print_performance

    arg0_1 = rand_strided((4, 3, 500, 400), (600000, 200000, 400, 1), device='cuda:0', dtype=torch.float32)
    return print_performance(lambda: call_v2([arg0_1]), times=times, repeat=repeat)


def benchmark_ref(times=15000, repeat=500):
    from torch._dynamo.testing import rand_strided
    from torch._inductor.utils import print_performance

    def ref_call(args):
        x = args[0]
        output = torch.nn.functional.interpolate(x, (256, 256), mode="bilinear", antialias=True, align_corners=False)
        return (output, )

    arg0_1 = rand_strided((4, 3, 500, 400), (600000, 200000, 400, 1), device='cuda:0', dtype=torch.float32)
    return print_performance(lambda: ref_call([arg0_1]), times=times, repeat=repeat)
...
if __name__ == "__main__":
    ...
    # print("benchmark_compiled_module_v2:")
    # compiled_module_main('None', benchmark_compiled_module_v2)

    print("benchmark_ref:")
    compiled_module_main('None', benchmark_ref)
