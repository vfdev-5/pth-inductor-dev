...
def call(args):
    arg0_1, = args
    args.clear()
    assert_size_stride(arg0_1, (16, 3, 500, 400), (600000, 200000, 400, 1))
    with torch.cuda._DeviceGuard(0):
        torch.cuda.set_device(0) # no-op to ensure context
        buf6 = empty((16, 3, 256, 256), device='cuda', dtype=torch.float32)
        # Source Nodes: [img], Original ATen: [aten.index, aten.mul, aten.sum]
        stream0 = get_cuda_stream(0)

        triton_per_fused_index_mul_sum_0.run(arg0_1, buf6, 256 * 256 * 3 * 16, 25, grid=grid(256 * 256 * 3 * 16), stream=stream0)
...
def benchmark_compiled_module(times=15000, repeat=500):
    ...
    arg0_1 = rand_strided((16, 3, 500, 400), (600000, 200000, 400, 1), device='cuda:0', dtype=torch.float32)
