...
class Repro(torch.nn.Module):
    ...
    def forward(self, arg0_1):
        iota = torch.ops.prims.iota.default(4, start = 0, step = 1, dtype = torch.int64, device = device(type='cpu'), requires_grad = False)
        view = torch.ops.aten.view.default(iota, [4, 1, 1, 1]);  iota = None
        ...
        iota_5 = torch.ops.prims.iota.default(4, start = 0, step = 1, dtype = torch.int64, device = device(type='cpu'), requires_grad = False)
        view_5 = torch.ops.aten.view.default(iota_5, [4, 1, 1, 1]);  iota_5 = None
...
def load_args(reader):
    buf0 = reader.storage(None, 1887840, dtype_hint=torch.uint8)
    reader.tensor(buf0, (4, 3, 345, 456), (471960, 1, 1368, 3), dtype=torch.uint8, is_leaf=True)  # arg0_1
