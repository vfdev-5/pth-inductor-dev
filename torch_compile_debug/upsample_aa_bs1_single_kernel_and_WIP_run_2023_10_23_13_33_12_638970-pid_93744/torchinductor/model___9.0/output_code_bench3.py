...
triton_per_fused_index_mul_sum_0_v3 = async_compile.triton('triton_6', '''
import triton
import triton.language as tl
from torch._inductor.ir import ReductionHint
from torch._inductor.ir import TileHint
from torch._inductor.triton_heuristics import AutotuneHint, persistent_reduction, pointwise
from torch._inductor.utils import instance_descriptor
from torch._inductor import triton_helpers

print("Compile triton_per_fused_index_mul_sum_0_v3")
...
@pointwise(
    size_hints=[262144],
    filename=__file__,
    meta={'signature': {0: '*fp32', 1: '*fp32', 2: 'i32'}, 'device': 0, 'device_type': 'cuda', 'constants': {}, 'mutated_arg_names': [], 'autotune_hints': set(), 'kernel_name': 'triton_per_fused_index_mul_sum_0_v3', 'configs': [instance_descriptor(divisible_by_16=(0, 1, 2), equal_to_1=(), ids_of_folded_args=(), divisible_by_8=(2,))]},
    min_elem_per_thread=0
)
@triton.jit
def triton_6(in_ptr0, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    xnumel = 196608
    xoffset = tl.program_id(0) * XBLOCK
    ...
    tmp189 = tl.full([XBLOCK], 1.0, tl.float32)
    tl.store(out_ptr0 + (x7), tmp189, None)
''')
...
def call(args):
    ...
    with torch.cuda._DeviceGuard(0):
        ...
        triton_per_fused_index_mul_sum_0.run(arg0_1, buf6, 25, 196608, grid=grid(196608), stream=stream0)
        del arg0_1
        return (buf6, )
...
def call_v3(args):
    ...
    with torch.cuda._DeviceGuard(0):
        ...
        triton_per_fused_index_mul_sum_0_v3.run(arg0_1, buf6, 256 * 256 * 3, grid=grid(256 * 256 * 3), stream=stream0)
...
if __name__ == "__main__":
    ...
    torch.manual_seed(12)
    arg0_2 = rand_strided((1, 3, 500, 400), (600000, 200000, 400, 1), device='cuda:0', dtype=torch.float32)

    torch.manual_seed(12)
    arg0_3 = rand_strided((1, 3, 500, 400), (600000, 200000, 400, 1), device='cuda:0', dtype=torch.float32)
    ...
    results.append(
        benchmark.Timer(
            ...
            num_threads=torch.get_num_threads(),
            label=f"Interpolate bilinear, AA=true, cuda",
            sub_label=f"Input (1, 3, 500, 400) -> 256, 256, {arg0_3.dtype}, CF",
            description=f"Compiled v3",
        ).blocked_autorange(min_run_time=min_run_time)
    )
    results.append(
        benchmark.Timer(
            ...
            num_threads=torch.get_num_threads(),
            label=f"Interpolate bilinear, AA=true, cuda",
            sub_label=f"Input (1, 3, 500, 400) -> 256, 256, {arg0_2.dtype}, CF",
