        ...
        for(long i0=static_cast<long>(0L); i0<static_cast<long>(3L); i0+=static_cast<long>(1L))
        {
            ...
            for(long i1=static_cast<long>(0L); i1<static_cast<long>(32L); i1+=static_cast<long>(1L))
            {
                ...
                for(long i2=static_cast<long>(0L); i2<static_cast<long>(12L); i2+=static_cast<long>(1L))
                {
                    ...
                    auto tmp7 = static_cast<float>(1.00000000000000*i1);
                    ...
                    out_ptr0[static_cast<long>(i2 + (12L*i1) + (384L*i0))] = tmp146;
                }
            }
        }
    }
...
def call(args):
    ...
    buf0 = empty_strided((1, 3, 32, 12), (1152, 384, 12, 1), device='cpu', dtype=torch.float32)
...
def func():
    ...
    output = call([x, ])[0]
    expected = torch.nn.functional.interpolate(x, size=(32, 12), mode="bicubic", antialias=False)

    x_np = x[0].permute((1, 2, 0)).numpy()
    expected_cv2 = cv2.resize(x_np, (12, 32), interpolation=cv2.INTER_CUBIC)
