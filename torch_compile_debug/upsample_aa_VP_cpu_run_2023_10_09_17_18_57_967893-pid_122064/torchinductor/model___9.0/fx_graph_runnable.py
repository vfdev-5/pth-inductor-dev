...
class Repro(torch.nn.Module):
    ...
    def forward(self, arg0_1):
        iota = torch.ops.prims.iota.default(271, start = 0, step = 1, dtype = torch.int64, device = device(type='cpu'), requires_grad = False)
        add = torch.ops.aten.add.Tensor(iota, 0.5);  iota = None
        mul = torch.ops.aten.mul.Tensor(add, 1.2730627306273063);  add = None
        sub = torch.ops.aten.sub.Tensor(mul, 1.2730627306273063)
        ...
        add_2 = torch.ops.aten.add.Tensor(mul, 1.2730627306273063)
        ...
        clamp_max = torch.ops.aten.clamp_max.default(convert_element_type_1, 345);  convert_element_type_1 = None
        ...
        iota_1 = torch.ops.prims.iota.default(5, start = 0, step = 1, dtype = torch.int64, device = device(type='cpu'), requires_grad = False)
        ...
        mul_1 = torch.ops.aten.mul.Tensor(add_5, 0.7855072463768116);  add_5 = None
        ...
        iota_2 = torch.ops.prims.iota.default(5, start = 0, step = 1, dtype = torch.int64, device = device(type='cpu'), requires_grad = False)
        ...
        clamp_max_3 = torch.ops.aten.clamp_max.default(add_6, 344);  add_6 = None
        ...
        slice_3 = torch.ops.aten.slice.Tensor(slice_2, 3, 0, 9223372036854775807);  slice_2 = None
        index = torch.ops.aten.index.Tensor(slice_3, [None, None, clamp_max_3]);  slice_3 = clamp_max_3 = None
        unsqueeze_2 = torch.ops.aten.unsqueeze.default(div, -1);  div = None
        mul_2 = torch.ops.aten.mul.Tensor(unsqueeze_2, index);  unsqueeze_2 = index = None
        sum_2 = torch.ops.aten.sum.dim_IntList(mul_2, [-2]);  mul_2 = None
        clone = torch.ops.aten.clone.default(sum_2, memory_format = torch.channels_last);  sum_2 = None
        return (clone,)
        
def load_args(reader):
    buf0 = reader.storage(None, 7551360)
    reader.tensor(buf0, (4, 3, 345, 456), (471960, 1, 1368, 3), is_leaf=True)  # arg0_1
