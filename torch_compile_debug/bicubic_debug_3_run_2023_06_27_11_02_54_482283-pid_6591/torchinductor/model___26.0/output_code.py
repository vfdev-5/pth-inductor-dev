...
{
    {
        ...
        for(long i0=static_cast<long>(0L); i0<static_cast<long>(3L); i0+=static_cast<long>(1L))
        {
            ...
            for(long i1=static_cast<long>(0L); i1<static_cast<long>(12L); i1+=static_cast<long>(1L))
            {
                ...
                {
                    auto tmp0 = static_cast<scalar_t>(1.00000000000000*i2);
                    auto tmp1 = std::floor(tmp0);

                    assert(tmp0 == tmp1);

                    auto tmp2 = tmp0 - tmp1;
                    auto tmp3 = static_cast<scalar_t>(1.0);
                    auto tmp4 = min_propagate_nan(tmp3, tmp2);
                    auto tmp5 = static_cast<scalar_t>(0.0);
                    auto tmp6 = max_propagate_nan(tmp5, tmp4);

                    assert(tmp6 == 0.0);

                    auto tmp7 = static_cast<scalar_t>(0.833333333333333 + (2.66666666666667*i1));
                    auto tmp8 = std::floor(tmp7);
                    auto tmp9 = tmp7 - tmp8;
                    auto tmp10 = min_propagate_nan(tmp3, tmp9);
                    auto tmp11 = max_propagate_nan(tmp5, tmp10);

                    auto tmp12 = static_cast<int>(tmp8);
                    auto tmp13 = static_cast<int>(tmp1);
                    ...
                    auto tmp39 = static_cast<scalar_t>(-0.75);
                    auto tmp40 = decltype(tmp39)(tmp39 * tmp38);
                    auto tmp41 = static_cast<scalar_t>(-3.75);
                    auto tmp42 = tmp40 - tmp41;
                    auto tmp43 = decltype(tmp42)(tmp42 * tmp38);
                    auto tmp44 = static_cast<scalar_t>(-6.0);
                    auto tmp45 = tmp43 + tmp44;
                    auto tmp46 = decltype(tmp45)(tmp45 * tmp38);
                    auto tmp47 = static_cast<scalar_t>(-3.0);
                    auto tmp48 = tmp46 - tmp47;
                    auto tmp49 = static_cast<scalar_t>(1.25);
                    auto tmp50 = decltype(tmp49)(tmp49 * tmp6);
                    auto tmp51 = static_cast<scalar_t>(2.25);
                    ...
                    out_ptr0[static_cast<long>(i2 + (32L*i1) + (384L*i0))] = tmp146;
                }
            }
        }
    }
...
def call(args):
    ...
    buf0 = empty_strided((1, 3, 12, 32), (1152, 384, 32, 1), device='cpu', dtype=torch.float32)
...
def func():
    ...
    output = call([x, ])[0]
    expected = torch.nn.functional.interpolate(x, size=(12, 32), mode="bicubic", antialias=False)

    x_np = x[0].permute((1, 2, 0)).numpy()
    expected_cv2 = cv2.resize(x_np, (32, 12), interpolation=cv2.INTER_CUBIC)
